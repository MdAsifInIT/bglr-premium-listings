...
import { getSampleListingById } from "@/lib/sample-listings";
...
type PropertyDetails = NonNullable<Awaited<ReturnType<typeof getPropertyById>>["data"]["property"]> & {
  isDemo?: boolean;
};

function PropertyDetailsView({ property }: { property: PropertyDetails }) {
  return (
            ...
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-8">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {property.isDemo && (
                    <span className="rounded-md bg-amber-100 px-2 py-1 text-[10px] font-semibold uppercase tracking-wider text-amber-800 dark:bg-amber-950/50 dark:text-amber-300">
                      Demo
                    </span>
                  )}
                </div>
                <div className="flex flex-wrap items-center gap-3 mt-2 text-zinc-600 dark:text-zinc-400 text-sm">
...
export default async function PropertyPage({ params }: PropertyPageProps) {
  ...
  const sampleProperty = getSampleListingById(id);

  if (sampleProperty) {
    return <PropertyDetailsView property={sampleProperty as PropertyDetails} />;
  }
