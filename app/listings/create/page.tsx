...
export default function CreateListingPage() {
  ...
  if (authLoading) return <div className="flex-1 flex items-center justify-center bg-zinc-950 text-zinc-400">Verifying session...</div>;
  if (!user) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 bg-zinc-950 text-center">
        <h2 className="font-serif text-2xl font-semibold mb-3">Access Restricted</h2>
        <p className="text-zinc-400 mb-6">You must be logged in to create listing files.</p>
        <Link href="/auth/login" className="bg-emerald-800 hover:bg-emerald-700 px-6 py-2.5 rounded-lg font-medium transition-colors">Sign In</Link>
      </div>
    );
  }
    ...
    <div className="flex-1 flex items-center justify-center p-6 bg-zinc-950">
      <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-xl max-w-lg w-full shadow-2xl">
        <div className="flex items-center justify-between border-b border-zinc-800 pb-4 mb-6">
          <h2 className="font-serif text-xl font-semibold text-zinc-100">Create Premium Listing</h2>
          <span className="text-xs text-zinc-500 font-mono">Step {step} of 4</span>
        </div>
        {error && <div className="bg-red-950/50 border border-red-900/50 text-red-400 p-3 rounded-lg text-sm mb-6">{error}</div>}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            {step === 4 && (
              <div className="space-y-4">
                <label className="block text-xs font-semibold text-zinc-300 uppercase tracking-wider mb-2">Upload Assets</label>
                <Controller
                  name="imageUrls"
                  control={control}
            ...
            <div className="flex items-center justify-between pt-4 border-t border-zinc-800">
              {step > 1 ? (
                <Button type="button" variant="secondary" onClick={() => setStep((s) => s - 1)} disabled={submitting} className="px-4 py-2 border border-zinc-800 hover:bg-zinc-800 text-zinc-300 rounded-lg text-sm transition-colors cursor-pointer disabled:opacity-50">Back</Button>
              ) : <div />}
              {step < 4 ? (
                <Button type="button" onClick={nextStep} className="bg-emerald-800 hover:bg-emerald-700 text-zinc-100 font-semibold px-5 py-2 rounded-lg text-sm transition-colors cursor-pointer">Continue</Button>
              ) : (
                <Button type="submit" loading={submitting} className="bg-emerald-800 hover:bg-emerald-700 active:bg-emerald-900 text-zinc-100 font-semibold px-5 py-2 rounded-lg text-sm transition-colors cursor-pointer disabled:opacity-50 flex items-center gap-2">Publish Listing</Button>
              )}
            </div>
          </form>
        </FormProvider>
